...
# LLM annotations need LLM_API_KEY in .env (Anthropic, a workspace-scoped key or LLM_WORKSPACE_ID; default model claude-sonnet-5)
uv run offramp annotate --from <extract> [--limit N]   # annotate an existing scan, no org calls; xray --annotations <file> reuses it by content hash
uv run offramp review sample|score|apply   # human review sample → calibration.json → calibrated_confidence on every annotation
make test         # pytest unit + integration
make lint         # ruff format + ruff check
make typecheck    # mypy --strict
make smoke        # end-to-end smoke test against scratch org
make apex-parser  # install the grammar-backed Apex parser (Node); without it the tokenizer is used
uv run offramp health --from <extract>     # static checks (dead picklist values, callouts in saves, owner alerts…)
uv run offramp recipes --from <extract> --out recipes.json   # generate verify recipes; verify --auto-recipes does it inline
...
- **Apex analysis** in `src/extract/apex` behind the `ApexAnalysis` contract (AD-31): the grammar-backed engine (Salesforce's ANTLR grammar via `@apexdevtools/apex-parser`, Node, `tools/apex-parser`, `make apex-parser`) is the default and the own tokenizer is the fallback; summit-ast is archived, do not add it; regex classifiers for LWC (`src/extract/lwc`) and Aura (`src/extract/aura`); own Flow XML normalizer in `src/extract/categories/flow.py`
...
- **AD-31**: No summit-ast; Apex analysis behind the `ApexAnalysis` contract — grammar-backed (ANTLR, Node) by default, tokenizer fallback (`OFFRAMP_APEX_ENGINE=auto|ast|tokenizer`)
...
26. **SFDX source trees are multi-root, suffix-typed, and contain decoys.** A package directory (`sfdx-project.json` → `packageDirectories[].path`) may hold metadata in *any* subfolder and the type comes from the file suffix, not the folder: NPSP keeps its trigger framework in `force-app/tdtm/classes/`, apex-recipes its tests in `force-app/tests/<Group>/` with no `classes/` folder at all. `SourceTree` uses package directories as roots and recursive suffix globs (`**/*.cls`), merges objects split across packages, strips CumulusCI namespace tokens (`%%%NAMESPACE%%%`), and skips `.git`/`node_modules`/`.sfdx`/`.sf` (`.git/objects` looks like a metadata folder). Reading only `main/default` found 35 of 139 apex-recipes classes and lost 221 NPSP components. `sfdx-project.json` lists several `packageDirectories`, each with its own `main/default`, and one object can be split across them (Easy Spaces defines `Reservation__c` in `es-base-objects` and adds fields in `es-base-code`); `.git/objects` looks like a metadata `objects/` folder to a naive glob. `SourceTree.roots` reads every package root, merges split objects, and skips `.git`/`node_modules`/`.sfdx`/`.sf`. Custom metadata *records* live in `customMetadata/<Type>.<Record>.md-meta.xml` and are read by `read_cmt_records_from_source`.
...
31. **Big real corpora (NPSP 1,334 components, EDA 814) taught three parser rules.** (a) Fields and objects with a *foreign* namespace prefix (`npo02__Household__c`, `npe01__OppPayment__c`) belong to installed packages a source tree cannot contain: they become inferred fields tagged with the package and one `Package` external node per namespace (`summary.package_dependencies`), not unresolved references — NPSP went from 2,743 unresolved to 431 and reports its five base packages. (b) ALL_CAPS identifiers are constants, and `AggregateResult`/`HttpCalloutMock`/`SaveResult`/`AccessLevel`/… are platform types; standard sObject names used as Apex types (`EntityDefinition`, `LeadStatus`) are object references. (c) TDTM-style frameworks register handlers in **Apex**, not metadata: `new Trigger_Handler__c(Class__c='X', Object__c='Contact', Trigger_Action__c='AfterInsert')` rows are extracted by the analyzer (`dispatch_rows`), join the CMT rows, and dispatch from the trigger on `Object__c` (88 NPSP / 90 EDA dispatch edges). Still open at scale: enum values and inner types used through inheritance, lowercase variables read as objects (`address.MailingCity__c`), fflib-style library classes reached only through interfaces.
32. **The Apex analyzer has two engines; the grammar one is the default.** `offramp.extract.apex.analyze` parses through Salesforce's ANTLR grammar (`tools/apex-parser/apex_ast.js`, a persistent Node process started once per Python process by `ast_bridge`, ~9 s for NPSP's 1,061 files) and walks the tree in `ast_analyzer.py`; the tokenizer in `references.py` is the fallback when Node or the package is missing or a file has syntax errors (`ApexAnalysis.engine` / `parse_errors` say which). What the tree fixes: variables are typed by declaration with block scoping (`address.MailingCity__c` → `Address__c`, `Trigger.new` → the trigger's object, `map.get(k).Field`), a member named like another class (`ContactSelector contactSelector`) no longer hides that class, enum constants / inner types / `when` values are never class references, `System.Label.X`, `insert new X(...)`, `upsert x External__c`, `X.SObjectType.class` are recognised, and a variable's type beats the case-insensitive class-candidate guess. NPSP unresolved went 431 → 13, EDA 135 → 0, apex-recipes 22 → 0, and ~2,300 tokenizer edges to *fields mistaken for objects* disappeared. Both engines share the vocabulary in `references.py` (`STANDARD_SOBJECTS`, `_SYSTEM_NAMESPACES`, `_parse_soql`, `_fold`); add new platform types there so both agree. `tests/unit/test_apex_analyzer.py` runs every case against both engines.

33. **Static health checks and generated recipes close the loop without a person in it.** `offramp health` (`understand/health.py`, also `health.json` on every extract and a section + tile in the report) runs six deterministic rules over the process model, the Apex analysis and the schema: `picklist_value_unknown` (entry conditions, decision outcomes, step filters and assignments compared with the field's picklist values; rule criteria list alternatives as `Gold, Platinum`, multi-selects as `A;B`), `callout_in_save_path` (record-triggered flow or trigger → invocable class with `Http*`/`WebServiceCallout` and no `System.enqueueJob`/`@future`, one delegation hop deep; `callout_deferred` info when an async job is enqueued), `owner_alert_after_queue_assignment`, `same_object_update_after_save` (warning without entry conditions, info with), `multiple_triggers_per_object`, `no_fault_path`. First real catch: the fixture's NightlyHousekeeping deletes leads with `Status = 'Dead'`, a value the picklist does not have. `offramp recipes` / `verify --auto-recipes` (`verify/recipes.py`) synthesise a recipe per verifiable flow: required fields by type (skipping compound `Name` when `LastName` exists and anything the describe marks non-createable), entry conditions satisfied on create — or on the update step for update-only flows, where `requires_change` also strips those values from the create — `ISBLANK(Field)` terms of the object's validation rules filled, sibling record-triggered flows kept quiet by picking another picklist value, `recordId`-style inputs of autolaunched flows paired with a fresh record of the first lookup's object. What it cannot decide lands in `_needs`. Live on 2026-09-11: both Developer Edition flows passed on generated recipes alone.

34. **Annotations read a dossier, and confidence is earned, not declared.** The first LLM pass (2026-09-11) sent a 4,000-character slice of raw metadata and asked for a number: mean 0.75, and the low scores sat exactly where the model had been given nothing (217 sharing rules as bare config, LWC/Aura without their sources, managed code with no body, 23 prompts truncated). `understand/annotate_context.py` now builds a *dossier* per component — identity + installed package (`packages.json`, from `InstalledSubscriberPackage`), FACTS from static analysis (entry points, callouts, async, DML/SOQL, fields written, complexity drivers, health findings, `offramp verify --json` results, and the deterministic tier hint from `understand/tier_rules.py`), the `ProcessDefinition` as Markdown, the dependency neighbourhood (callers, callees, data), and the SOURCE last (Apex body; LWC/Aura `raw["sources"]`, kept by the extractors up to 24 KB per bundle; normalised flow JSON), the only section truncated to the 30K budget and marked when it is. The model must return `evidence` quotes and `unknowns`; `_earned_confidence` starts from its `self_confidence`, adds 0.05 when its tier agrees with the hint, caps at 0.6 + `needs_review` when it contradicts it, then applies hard ceilings: source hidden 0.5, truncated 0.75, unknowns named 0.8, no evidence 0.5. What is knowable without a model is not asked: an empty sharing-rules file (216 of 217 in the DE) becomes a deterministic annotation at 1.0 (`model="rules"`); managed code and managed bundles (`(hidden)` sources) are described from the outside — package, version, `IsValid`, size, callers — and capped, never guessed. Clusters with 2+ annotated members get a `ProcessAnnotation` narrative (`process_annotations.json`, shown in the Business processes table with risks). Model JSON is parsed with `strict=False` and `\'` repaired, with one retry. Result on the DE: mean 0.75 → 0.92 overall (model-annotated rows 0.74 with the caps doing their job; managed components sit at 0.3–0.45 with their unknowns listed); 2 tier contradictions flagged for review, both legitimate (a queueable that only publishes an event; a UI `openRecordAction` mistaken for external work, since fixed in the rules). `offramp annotate --from <extract> [--processes] [--name X] [--dump-context DIR]` reruns without org calls and merges into the existing file.

35. **Calibration comes from people, and it measures accuracy, not knowledge.** `offramp review sample` picks a stratified set (every confidence band, the flagged and hidden-source cases, two rule-answered ones); reviewers answer summary yes/partly/no and tier yes/no; `offramp review score` writes `calibration.json` (accuracy per band blended with the band's mean confidence by n/(n+2), Brier score, over-/under-confident cases) and `review apply` adds `calibrated_confidence`, which the report shows in place of the rule value. First round on the DE (2026-09-11, 20 items): 20 of 20 summaries and tiers accepted, Brier 0.169, the rules were too harsh at the low end (0.0–0.5 band measured 1.00). Two lessons baked in: (a) a reviewer accepting "source hidden, likely a setup panel" confirms the sentence is *accurate*, not that the component is *known*, so hidden-source rows keep the 0.5 ceiling under calibration (`HIDDEN_SOURCE_CEILING`); (b) a sample with no rejections cannot measure the top end — the next round must ask about the narrative step by step on the largest, tier-2 and most-connected components, where errors would actually hide.
